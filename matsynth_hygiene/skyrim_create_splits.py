...
def minority_tile_index(mask_path, size=256, stride=128):
    ...
    for y in range(0, mask_array.shape[0] - size + 1, stride):
        for x in range(0, mask_array.shape[1] - size + 1, stride):
            ...
            for idx, color in CLASS_PALETTE.items():
                ...
                class_counts[CLASS_LIST[idx]] = np.sum(mask_color)
                total += class_counts[CLASS_LIST[idx]]

            ceramic_fraction = class_counts["ceramic"] / total if total > 0 else 0
            if ceramic_fraction >= TAU and ceramic_fraction <= 0.3:
...
def main():
    ...
    print(
        f"\nCeramic heterogeneous ( 3/1.5/1% <= freq < 30% ) crops found: 256x256: {total_256}, 512x512: {total_512}, 768x768: {total_768}"
    )
